    ...
    for dataset in args.datasets:
        ...
        for sample in tqdm(loader, disable=True):
            ...
            if (
                ...
                (save_data_dir / f'{dataset}_duration.txt').exists() and
                (save_data_dir / f'{dataset}_resilience.txt').exists()
            ):
                continue
            ...
            resilience = load_resilience(args, total_travel_time, disrupted_travel_time, save_path)

            duration = time.time() - _start_time
            (save_data_dir / f'{dataset}_duration.txt').write_text(str(duration)) 
            (save_data_dir / f'{dataset}_resilience.txt').write_text(str(resilience)) 
            _logger.info(f"Took {duration} seconds to calculate the {dataset}'s resilience of {resilience}")
